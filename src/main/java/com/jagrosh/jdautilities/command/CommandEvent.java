...
public class CommandEvent {
    ...
    public void replyInDm(MessageEmbed embed, String message) {
        if (event.isFromType(ChannelType.PRIVATE))
            ...
            event.getAuthor().openPrivateChannel().queue(pc -> pc.sendMessage(message, embed)
                .queue());
        }
    }
    ...
    public void replyInDm(String message, MessageEmbed embed, Consumer<Message> success, Consumer<Throwable> failure) {
        if (event.isFromType(ChannelType.PRIVATE))
            getPrivateChannel().sendMessage(message, embed).
                queue(success, failure);
        else {
            event.getAuthor().openPrivateChannel().queue(pc ->
                pc.sendMessage(message, embed).queue(success, failure), failure);
        }
    }
