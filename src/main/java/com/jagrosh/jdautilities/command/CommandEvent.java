...
public class CommandEvent {
     ...
     * @param client The {@link com.jagrosh.jdautilities.command.CommandClient CommandClient}
     */
    ...
    public void reply(String message, MessageEmbed embed) {
        Message msg = new MessageBuilder().setContent(message)
            .setEmbed(embed)
            .build();
    ...
    public void replyInDm(MessageEmbed embed, String message) {
        Message msg = new MessageBuilder().setContent(message)
            .setEmbed(embed)
            .build();
        if (event.isFromType(ChannelType.PRIVATE))
            ...
            event.getAuthor().openPrivateChannel().queue(pc -> pc.sendMessage(msg)
                .queue());
        }
    }
    ...
    public void replyInDm(String message, MessageEmbed embed, Consumer<Message> success, Consumer<Throwable> failure) {
        Message msg = new MessageBuilder().setContent(message)
            .setEmbed(embed)
            .build();
        if (event.isFromType(ChannelType.PRIVATE))
            getPrivateChannel().sendMessage(msg).
                queue(success, failure);
        else {
            event.getAuthor().openPrivateChannel().queue(pc ->
                pc.sendMessage(msg).queue(success, failure), failure);
        }
    }
